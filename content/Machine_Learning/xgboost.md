...
假定序列的大小为N，给定误差ε和rank=r，其ε−approximate分位点为序列中的一些元素，且这些元素的rank值r′满足：$r′∈[r−εN,r+εN]$。以下面的序列为例，给定$ε=0.1,ϕ=0.5$，此时r=0.5×16=8，则rank值在区间[6.2,9.6][6.2,9.6]的元素为：{6,7,8}，即： *0.1-appoximate 0.5-quantile*为：{6,7,8} 
